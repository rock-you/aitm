...
===== Standards, frameworks, and methods, and the innovate/standardize/commoditize cycle
...
It is critical to realize that the frameworks tend to be lengthy and verbose. The ISO/IEC standards are brief by comparison, perhaps on average 10% of the corresponding framework.

Finally, methods (aka methodologies) in general are more action oriented and prescriptive. Scrum and XP are methods. It is at least arguable that PMBOK is a method as well as a framework. There is little industry consensus on some of these definitional issues and the student is advised to not be overly concerned about such abstract debates.
...
Ultimately, a cycle can be seen:
