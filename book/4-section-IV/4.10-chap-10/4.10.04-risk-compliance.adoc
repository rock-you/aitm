...
As noted above, ISACA replaced the term "controls" with "enablers," which we have already discussed. In their guise as controls, enablers such as policies, procedures, organizational structures, and the rest are used and intended to ensure that:
...
NOTE: In this chapter section we'll continue to use the generic term control as ultimately COBIT did not provide a good, terse substitute. "Testing enablers" is not how auditors talk.

Take a risk, such as the risk of a service (e.g. ecommerce Web site) outage resulting in loss of critical revenues.
