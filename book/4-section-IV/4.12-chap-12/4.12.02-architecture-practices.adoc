...
Note that flowcharts, data models, and other such diagrams tend to be associated more with the idea of "solutions" or "software" architecture. (We'll cover the xref:architecture-domains[architecture domains] in the next chapter section.)
...
Finally, enterprise architecture does not solely consist of diagrams; per TOGAF, the basic artifacts are catalogs, diagrams, and matrices and the act of drawing pictures itself may be the least important thing the architect does . The picture, if valuable, concisely represents a shared mental model on how the enterprise will undertake complex activities, and the discussions and negotiations the architect facilitates on the journey to such agreement are the real added value.
