...
Ideally, planning and analysis occurs "upstream" of the creation of a product team. In that guise, architecture functions as a sort of zoning or planning authority, and ideally there is no conflict with product teams because once the team is formed, the architect's job is done. However, this assumes that all the planning associated with launching a new product or capability was done correctly, and this itself is a kind of waterfall assumption. Some form of feedback and coordination is required in xref:multi-product-scaling[multi-product environments].

It is in the "governance and approval" kind of activity that conflict is most likely to emerge. Solving this rationally requires an economic model, such as establishing a nominal or xref:portfolio-CoD[portfolio-level Cost of Delay]. Like other processes, architecture itself can be assessed against such a baseline.

** Is enterprise cost of delay understood?
 We need to write this section, in Chapter 9, and cross-reference.
...
Of course, there are other ways architecture might add value beyond system replacement, in which case the strangler pattern may not be relevant.
...
** architect credibility: the *hands-on* problem (sidebar -- use "ground truth")
...
** The ultimate target op con for architecture: show economic value
...
* conclusion: If the cost of operating the coordination mechanism exceeds the value it provides, then continuing to operate it is irrational.

but arch re-emerges always - as humans we think in abstractions and the value drivers for arch continually surface
