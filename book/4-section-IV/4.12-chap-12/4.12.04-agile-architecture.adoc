...
* Curating the shared mental model ... common ground "Joint activity depends on interpredictability of the participants’ attitudes and actions. Such interpredictability is based on common ground — pertinent knowledge, beliefs and
 assumptions that are shared among the involved parties."

* the architecture of architecture, of the digital pipeline itself
** Arch - like staff functions generally - is a coordination mech
** The economic framework is essential
** WHEN does arch add value: coordination problems, gap problems
** understanding the nature of the work: flower v cog
** should "architecture" even be something experienced by product teams?

* Create the right context for behavior to emerge
** Prioritize team value
** Impacts: constraining choices can empower - minimize R&D
** Support the common ground
** Understand the risks of coordination mechs
** Optimize flow

* Architecture Kata & strangler pattern
** Identifying target conditions - good role for archs - impact model backref
** multi-constraint optimization
** e.g. "reduce tech debt" through supplier mgmt
** enable agility etc
** How are results to be evaluated - unused archs / patterns

* Execution (of what? shared arch svcs? decisions?)
** Is enterprise cost of delay understood? We need to write this section, in Chapter 9, and cross-reference.
** The nature of arch work
** what the arch is doing and whether they are blocking the work
** Are they servicing a queue? Or developing a product?
** Having a bench of archs that are assigned to collaborate with teams is generally uncontroversial. It's when archs are assigned governance (higher-order) responsibilities. Especially w/r/t design

** ticketed approvals only one way of solving issues
** market-based approaches - put in economic disincentives - project budget charged for new product onboarding - impersonal mechanics

* Arch as design
** HOW products are assembled into a platform - cannot be impersonal - hard design problem - requires teaming - CoD needs to inform the right people working on it at the right time
** architect credibility: the *hands-on* problem (sidebar)
** direct engagement and trust-building - harder to scale - Guilds

* outcomes
** product first
** artifacts & documentation
** digital exhaust - test/learning -
** inherently longer cycles in arch "experiments" - validation of effectiveness

* conclusion: arch re-emerges always - as humans we think in abstractions and the value drivers for arch continually surface

So how do we reconcile Agile with architecture practices, especially enterprise architecture and its concerns for longer lifecycles, aggregate technical debt, and governance?

One principle that should be apparent through this book has been "respect the team," because true product value originates there. If teams are constantly fragmented by enterprise operating models and governance mandates, their ability to creatively solve business problems is hampered. Enterprise architecture should take as a fundamental principle the protection of this precious resource that is the high-performing, collaborative, creative team. And while it's inevitable that the team will be subject to organization-wide mandates, they should be given the benefit of the doubt when autonomy collides with standardization.

When enterprise architecture takes on true business architecture questions, including how capabilities are to be enabled and enhanced, Agile insights actually become an input or kind of requirement to business architecture. What capabilities require high-performing, cross-functional teams? What capabilities can be supported with project-based temporary teams? It seems likely that the more valuable and difficult the work, the more it calls for the careful development of a common mental model among a close-knit team over time. Driving organizational capability investment into long-running team structures becomes a strategy that organizational architects should consider as they develop the overall organizational portfolio.
...
====== gap analysis vs test, learn, pivot
