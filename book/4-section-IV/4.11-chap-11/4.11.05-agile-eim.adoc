...
In a relational database, for example, indexing and operations such as joins will be significantly less efficient in such a structure. Also, there is nothing preventing the developer from mixing data in the tables, using the same columns to store different things.
...
This does not mean that the developer was off track. New approaches to data warehousing use generic schemas similar to what the developer was requesting. Speed of indexing and proper records management, can be solved in a variety of ways.
