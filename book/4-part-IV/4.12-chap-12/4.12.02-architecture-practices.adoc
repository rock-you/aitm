...
===== From the author: What do architects do, actually?
...
[[fig-gudea-350-o]]
.Gudea with blueprint, ~2140 BCE
image::images/4_12-gudea.jpg[blueprint, 350, , float="right"]
...
*Human vision uses parallel processing*. This means that a given image or visual stimulus is processed by many neurons simultaneously. This is how we can quickly recognize and act on threats, such as a crouching tiger (as in <<fig-tiger-350-o>> footnote:[Image https://www.flickr.com/photos/samiksha/2436037856, downloaded 2016-10-10, commercial use permitted]).
...
[[fig-neumann-flowchart-350-i]]
.The first software flowchart
image::images/4_12-neumann-flowchart.png[blueprint, 350, , float="left"]

Architects and architecture are known for creating diagrams -- abstract graphical representations of complex systems. The first known instance of applying graphical techniques to a digital problem was in 1947 (see <<fig-neumann-flowchart-350-i>> footnote:[From cite:[vonNeumann1947], public domain assumed.] and visual notations have evolved along with the field of computing ever since. Notable examples include:

* The IBM flowcharting template(see <<fig-flowchart-template-350-o>> footnote:[Image https://www.flickr.com/photos/mwichary/3249179483, , downloaded 2016-10-10, commercial use permitted])
...
* Harel state charts
* The Unified Modeling Language (see <<fig-sequence-diagram-450-i>> footnote:[Image https://www.flickr.com/photos/raphaelstolt/514643232, , downloaded 2016-10-10, commercial use permitted.])
...
[[fig-flowchart-template-350-o]]
.IBM flowcharting template
image::images/4_12-flowchart-template.jpg[blueprint, 350, , float="right"]
...
[[fig-sequence-diagram-450-i]]
.UML sequence diagram
image::images/4_12-sequence-diagram.png[blueprint, 450, , float="left"]
...
The big picture is part of the standard mindset of EA, which everyone immediately associates with the activities of an enterprise architect. However, many of these big pictures you meet in practice have been over-abstracted to the point of insignificance and no longer address any relevant question. cite:[Bente2012], p16.
...
For example, the illustration in <<fig-obesity-550-c>> footnote:[Image https://www.flickr.com/photos/pushandplay/2968259379, downloaded 2016-10-10, commercial use permitted], while intimidating, is likely useful to those who work with and study it. It would take some familiarization.

[[fig-obesity-550-c]]
.Complex diagram
image::images/4_12-obesity.jpg[blueprint, 550, ]

However,the diagram in <<fig-complex-network-550-c>> footnote:[Image https://www.flickr.com/photos/taedc/9614791576, downloaded 2016-10-10, commercial use permitted] is essentially unusable, as visually tracing any given line is too difficult, and it would be easy to mistakenly identify one bubble as dependent on another:

[[fig-complex-network-550-c]]
.Another complex diagram
image::images/4_12-complex-network.jpg[blueprint, 550, ]

This may seem like an obvious critique, but architectural diagrams of similar complexity and unusability have too often been produced. Another issue with some diagrams is that they do not give a good sense of perspective or scale. This is sometimes seen in the business architecture practice of "capability mapping." For example, suppose you see a diagram such as shown in <<fig-capabilities-600-c>>.

[[fig-capabilities-600-c]]
...
For example, consider the image shown in <<fig-proc-func-600-c>>.
...
Notice that a matrix (<<fig-proc-func-matrix-600-c>>) can be read in the same way.

[[fig-proc-func-matrix-600-c]]
...
When we establish a catalog of architectural entities, we are engaging in xref:system-of-record[master data management]. In fact, the architectural concepts can be represented as a form of xref:data-modeling[database schema] (see <<fig-simpleMetamodel-400-c>>).

[[fig-simpleMetamodel-400-c]]
.A simple metamodel
...
The challenge is, when does it make economic sense to pre-aggregate the data? The economic graph shown in <<fig-econRepo-800-c>> may assist in thinking about this.
...
[quote, Bente et al, Collaborative Enterprise Architecture]
One core question decided by governance is how much autonomy is granted to business units or geographical regions. In case this autonomy is high, would a quest for high IT integration and standardization not be like fighting windmills? cite:[Bente2012] p.45
...
[[fig-airbus-300-i]]
.Airbus A-380
image::images/4_12-A-380.jpg[A-380, 300, , float="left"]


In 2006, Airbus was in the midst of developing the gigantic A-380 aircraft. Because of the scale of the effort, multiple teams of engineers in different countries were working on the effort. As William Ulrich says of the German and French teams, "having failed to coordinate efforts to wire the world's largest plane, they were surprised to learn that they had used different approaches to designing the A380's wiring system" cite:[Ulrich2010], p. 32. This was because the teams were using different software for wiring design of the critical wiring design software cite:[Clark2006]. The use of different software to fulfill the capability resulted in a year's delay, a $4.5bn order loss to Boeing, and a 26% drop in Airbus stock.footnote:[Image credit https://www.flickr.com/photos/44400809@N07/4081438180, downloaded 2016-10-11, commercial use permitted.]

====== Technology rationalization case #2

Henrik Kniberg worked on the PUST project for the Swedish national police, which was a project to deploy laptops to the Swedish police. Because of its urgency, it was allowed to use Lean and Agile techniques, and received favorable media coverage and was generally deemed a success  cite:[Kniberg2011].

Unfortunately, a drive for technology "rationalization" resulted in the re-platforming of the system on a commercial platform (Siebel) cite:[Kniberg2013]. Furthermore, an xref:open-loop[open loop] xref:Agile-history[waterfall] method was used to deliver the system. Kniberg reports that the result has been an unsuccessful rollout, unhappy users, and cost to the Swedish police department of about 1 biliion pounds ($1.6B).
