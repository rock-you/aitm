...
In fact, data management capabilities often have sought to intervene with developers, sometimes placing procedural requirements upon the development teams when database services were required. This approach is seen in xref:1_03.02-Agile-history[waterfall] development and when database teams are organized xref:product-v-function[functionally].
