...
Process models can become extremely complex, and are seen describing both human and automated activity. Notice how different it is from Kanban. In Kanban, everything is a work item, and the overall flow is some simple version of "to do, doing, done." This can become very complex when the flow gets more elaborate (e.g. various forms of testing, deployment checks, etc.) In a process model, the activity is explicitly specified, on the assumption it will be repeated. The boxes representing steps are essentially equivalent to the columns on a Kanban board, and since sticky notes are not being used, process models can become very complex. Process modeling is discussed in detail in the xref:process-modeling[appendix]. Process management as a practice is discussed extensively in Part III. However, before we move on, two simple variations on process management are:
