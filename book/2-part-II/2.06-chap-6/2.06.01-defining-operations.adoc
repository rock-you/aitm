...
For this chapter we'll use the following definition of operations:

_A duty-oriented, physical presence in the service of executing particular tasks, that support broader organizational goals and strategies, and need to be systematically discharged._
...
The continuous cycle of innovation and commoditization has driven closer and closer ties between “development” and “operations.” This cycle also has driven confusion around exactly what is meant by “operations.” In many organizations, there is an “Infrastructure and Operations” function. Pay close attention to the naming. A matrix may help, because we have two dimensions to consider here.
...
. Once service is restored, investigate why the issue happened and propose longer-term solutions.
...
At the end of the day, we need to remember that operational work is just one form of work. In a single-team organization, these processes might still be informal and handled through simple xref:task-mgmt[task management] (although user provisioning would need to be automated, if the system  is scaling significantly.) We'll continue on that assumption for the remainder of this chapter, and revisit the idea of defined, repeatable processes as we scale to a "team of teams" in xref:Section-III-coordination[Part III].
