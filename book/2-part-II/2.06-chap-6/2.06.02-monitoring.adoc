...
image::images/2.06-monitoring-3.png[application server and monitor, 350,,float="left"]
...
image::images/2.06-monitoring-5.png[aggregation,300,,float="right"]
...
At a simpler level, capacity and performance management may consist of monitoring CPU, memory, and storage utilization across a given set of nodes, and raising alerts if certain threshholds are approached. Capacity analytics at its most advanced is a true Big Data problem domain.
