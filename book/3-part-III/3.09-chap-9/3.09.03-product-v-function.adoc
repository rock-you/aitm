...
Let's accept Scrum and the xref:amazon-productization[2-pizza team] as our organizing approach. A large scale Scrum effort is based on multiple small teams (i.e. representing xref:AKF-cube[AKF scaling cube partitions]). If we want to minimize xref:multi-tasking[multi-tasking and context-switching], we need to ask "how many product teams can a given product owner handle?" In _Agile Product Management with Scrum_, Roman Pichler says, "My experience suggests that a product owner usually cannot look after more than two teams in a sustainable manner" (<<Pichler2010>>, p. 12). Scrum authors therefore suggest that larger scale products be managed as aggregates of smaller teams. We'll discuss how the product structure is defined in Chapter 8.


anchor:shared-services[]

===== From components to shared services
 probably move this to chapter 9

In a world of digital products delivered as services, both feature and component teams may be the recipients of ongoing investment. One reason for component teams is the increasing depth of specialization seen in the evolving digital organization. As systems scale up, deeper and deeper skills are called for:

* Network engineers with advanced understandings of routing and security
* Database engineers with advanced skills in performance tuning and indexing
* Application architects with advanced skills in caching architectures

An ongoing objection in discussions of Agile is, "We can't put a specialist on every team!"


Lifecycle: collaboration, internal commoditization, external commoditization
 Effectiveness, efficiency, governance -Benchmarkable
...
Scenario: repetitive yet error prone work ** how function *becomes* product - my little data standards self-service example... team to team interactions that become "servicized" ...critical importance of self-service
