...
Team members also can be "available" to each other
...
*Cadenced approaches.* When a synchronization activity occurs on a timed interval, this can be called a xref:cadence[cadence]. Sometimes, cadences are layered; for example, a daily standup, a weekly review, and a monthly Scrum of Scrums. Reinertsen calls this harmonic cadencing (<<Reinertsen2009>> pp. 190-191).
...
A variety of boundary-spanning liaison and coordination structures are suggested by various authors like Mike Cohn, Atul Gawande, and others, including:
...
The above approaches imply synchronized meetings and face to face interactions. When the boundary-spanning approach is based on artifacts, we move into the realms of process and project management. Artifact-centric approaches often receive criticism for introducing too much latency into the product development process; they often imply xref:queues[queues], and sometimes flow through multiple queues in an uncoordinated way, as we warned in Chapter 5.
