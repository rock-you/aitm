...
In keeping with the xref:emergence-model[emergence model] thought experiment, another key state transition is from one product to multiple products. This kind of scaling is distinct from scaling one product through subdividing it into multiple features.
